...
class CyberpunkContactApp:
    ...
    def draw_scan_lines(self):
        ...
        self.root.after(100, self.draw_scan_lines)
    ...
    def start_binary_rain(self):
        ...
        for stream in self.binary_streams[:]:
            ...
            # 移除超出屏幕的流
            if stream['y'] - stream['length'] * 20 > self.root.winfo_height():
                self.binary_streams.remove(stream)

        self.root.after(50, self.start_binary_rain)
...
if __name__ == "__main__":
    ...
    def update_progress():
        for i in range(101):
            progress['value'] = i
            loading_text.config(text=f"加载系统组件... {i}%")
            splash.update()
            root.after(20)
        splash.destroy()
        root.after(100, lambda: CyberpunkContactApp(root))
